    ...
    try {
      ...
      try {
        const response = await fetch(
          `${config.api.url}?key=${config.api.key}`,
          {
            method: "POST",
            mode: "cors",
            headers: {
        ...
        if (!response.ok) {
          ...
          if (statusCode >= 500) {
            throw new Error(
              `Lỗi máy chủ AI (${statusCode}). Vui lòng thử lại sau.`,
            );
        ...
        try {
          ...
          if (
            textContent.includes('"isValidPortrait": true') ||
            textContent.includes('"isValidPortrait":true')
          ) {
            return { isValidPortrait: true, validationError: null };
          } else if (
            textContent.includes('"isValidPortrait": false') ||
            textContent.includes('"isValidPortrait":false')
          ) {
        ...
        if (fetchError.name === "AbortError") {
          ...
          throw new Error(
            "Không thể kết nối tới máy chủ AI. Vui lòng kiểm tra kết nối mạng và thử lại.",
          );
        }
        throw fetchError;
      }
      ...
      throw new Error(
        "Không thể xử lý phản hồi từ server AI. Vui lòng thử lại.",
      );
    }
  }
