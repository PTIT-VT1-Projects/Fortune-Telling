...
const AnalysisProgress = ({ file, image, onRetry, onAnalysisComplete }) => {
  ...
  /**
   * Validate image and generate 5 years later image.
   */
  useEffect(() => {
      ...
      try {
        ...
        try {
          ...
          if (!aiValidation.isValidPortrait) {
            const errorMessage =
              aiValidation.validationError || "Ảnh chân dung không hợp lệ";
        ...
        // Step 3: Analysis steps (90%)
        const analysisSteps = [
          { text: "Đang phân tích tổng quan", progress: 90 },
        ];
        ...
        document
          .querySelector(".analysis-badge")
          .classList.add("final-analysis");
        setLoadingProgress(90);

        try {
          let apiResult = null;
          let retryAttempts = 0;
          const maxApiRetries = 2; //Retry 2 times
          ...
          const apiResultWithImagePreview = {
            ...apiResult,
          };

          // Set the analysis result from the API
          setAnalysisResult(apiResultWithImagePreview);
        } catch (error) {
          setIsApiError(true);
        ...
        setValidationError(
          error.message || "Có lỗi xảy ra khi kiểm tra ảnh. Vui lòng thử lại.",
        );
        setLoadingProgress(20);
      }
    };
        ...
        <div className="analysis-content">
          ...
          <div className="analysis-loading">
            ...
            <div className="analysis-status">
              {statusText}
              <span className="animated-dots">{"".padEnd(dotsCount, ".")}</span>
            </div>
          </div>
        </div>
      </div>
