import AiService from "../pages/games/face-reading/services/aiFaceReadingService";
...
class ImageService {
  ...
  static simulateLoadingProgress(setLoadingProgress, loading, imageFile) {
    ...
    const progressPhases = [
      ...
      { progress: 20, portion: 0.1 }, // Image preparation - 10% of total time
      { progress: 30, portion: 0.1 }, // Sending to API - 10% of total time
      { progress: 40, portion: 0.05 }, // API received - 5% of total time
      { progress: 50, portion: 0.1 }, // API processing - 10% of total time
  ...
  static validateBasic(file) {
    ...
    if (!acceptedTypes.includes(file.type)) {
      return {
        isValid: false,
        error:
          "Định dạng ảnh không được hỗ trợ. Vui lòng sử dụng JPEG/JPG, PNG, BMP, HEIC, HEIF hoặc WEBP",
      };
    }
